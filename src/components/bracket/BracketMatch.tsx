...
export function BracketMatch({ match }: { match: MatchWithPlayers }) {
  const { player1, player2, scorePlayer1, scorePlayer2, winnerId, status } =
    match;

  const scoreDisplay =
    status === "COMPLETED" ? `${scorePlayer1 ?? 0}-${scorePlayer2 ?? 0}` : "-";

  const renderPlayer = (
    player: MatchWithPlayers["player1"] | MatchWithPlayers["player2"],
    isPlayer1: boolean,
  ) => {
        ...
        <div className="flex flex-col justify-start p-2 h-auto">
          <div className="flex items-center gap-3 mt-1">
            <Avatar className="h-8 w-8">
              ...
              <AvatarFallback>
                {player.user?.name?.charAt(0).toUpperCase() ?? "Q"}
              </AvatarFallback>
            </Avatar>
            <span className="font-medium text-primary break-words">
              {player.user?.name ?? "Qualifier"}
            </span>
          </div>
        </div>
      );
    ...
    return (
      <div className="flex justify-between items-center w-full">
        <div className="flex items-center gap-3">
          ...
          <span
            className={`font-medium ${isWinner ? "font-bold" : ""} ${textColor}`}
          >
            {player.user?.name ?? "Unknown Player"}
          </span>
        </div>
        <div
          className={`font-semibold text-lg ${isWinner ? "font-bold" : ""} ${textColor}`}
        >
