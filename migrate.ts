...
async function main(dryRun: boolean = false) {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      const oldPlayers = oldDb
        .prepare("SELECT * FROM Player")
        .all() as OldPlayer[];
        ...
        try {
          await tx.user.create({
            data: {
              ...
              email:
                oldPlayer.email ||
                `${oldPlayer.name.toLowerCase().replace(/ /g, "")}@example.com`,
              role: "PLAYER",
            },
          });
          await tx.player.create({
            data: {
              ...
              seed: oldPlayer.seed,
              status:
                oldPlayer.status === "eliminated" ? "ELIMINATED" : "ACTIVE",
            },
          });
      ...
      const oldMatches = oldDb
        .prepare("SELECT * FROM Fixture")
        .all() as OldMatch[];

      for (const oldMatch of oldMatches) {
        const player1 = await tx.player.findFirst({
          where: { seed: oldMatch.player1Id },
        });
        const player2 = await tx.player.findFirst({
          where: { seed: oldMatch.player2Id },
        });
        const winner = oldMatch.winnerId
          ? await tx.player.findFirst({ where: { seed: oldMatch.winnerId } })
          : null;
        ...
        try {
          const round = oldMatch.round
            ? (roundMap[oldMatch.round.toUpperCase().replace(/ /g, "_")] ??
              "ROUND_1")
            : "ROUND_1";
          ...
          const newMatch = await tx.match.create({
            data: {
              ...
              winnerId: winner?.id,
              status,
              scheduledTime: oldMatch.scheduledTime
                ? new Date(oldMatch.scheduledTime)
                : undefined,
          ...
          const oldFrames = oldDb
            .prepare("SELECT * FROM Frame WHERE fixtureId = ?")
            .all(oldMatch.id) as OldFrame[];
          if (oldFrames.length) {
            await tx.result.create({
              data: {
                matchId: newMatch.id,
                frameDetails: oldFrames.map((f) => ({
                  frame: f.frameNumber,
                  winner: f.winnerId,
                })),
              },
