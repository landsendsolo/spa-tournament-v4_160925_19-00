...
async function main() {
  ...
  correctDraw.forEach((matchData) => {
    if (matchData.p1 && !playerMap.has(matchData.p1))
      missingPlayers.add(matchData.p1);
    if (matchData.p2 && !playerMap.has(matchData.p2))
      missingPlayers.add(matchData.p2);
  });
  if (missingPlayers.size > 0) {
    console.error(
      `[ERROR] Missing players in DB for draw: \n - ${[...missingPlayers].join("\n - ")}\nPlease ensure they are in the seed script. Aborting.`,
    );
    process.exit(1);
  }
  ...
  if (tournaments.length === 0) {
    console.error(
      "[ERROR] No tournament found. Please run the seed script first. Aborting.",
    );
    process.exit(1);
  }
  if (tournaments.length > 1) {
    console.error(
      "[ERROR] Multiple tournaments found with the same name. Use ID or unique name. Aborting.",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    const roundCounts = createdMatches.reduce(
      (acc, match) => {
