...
async function getPlayerData(userId: string): Promise<PlayerData> {
  ...
  const allCompletedMatches = player
      ...
      return (
        <div className="text-center py-12">
          ...
          <p className="mt-4 text-muted-foreground">
            Your player profile could not be found in the tournament data.
          </p>
        </div>
      );
    }
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center py-12">
      ...
      <h2 className="text-4xl font-bold mb-4">
        SPA Scottish Singles Area Draw
      </h2>
        ...
        <div className="lg:col-span-2 space-y-8">
          {upcomingMatch ? (
              ...
              <CardContent>
                <div className="flex items-center justify-around text-center">
                  <div className="flex flex-col items-center gap-2">
                    <Avatar className="h-20 w-20">
                      <AvatarImage
                        src={upcomingMatch.player1?.user.image || ""}
                      />
                      <AvatarFallback>
                        {upcomingMatch.player1?.user.name
                          ?.charAt(0)
                          .toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
                  ...
                  <div className="flex flex-col items-center gap-2">
                    <Avatar className="h-20 w-20">
                      <AvatarImage
                        src={upcomingMatch.player2?.user.image || ""}
                      />
                      <AvatarFallback>
                        {upcomingMatch.player2?.user.name
                          ?.charAt(0)
                          .toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
                ...
                <div className="mt-6 text-center text-muted-foreground space-y-2">
                  <p className="flex items-center justify-center gap-2">
                    ...
                    {upcomingMatch.round.replace("_", " ")}
                  </p>
                  <p className="flex items-center justify-center gap-2">
                    <Calendar className="h-4 w-4" />{" "}
                    {upcomingMatch.scheduledTime
                      ? new Date(upcomingMatch.scheduledTime).toLocaleString()
                      : "TBD"}
                  </p>
                  <p className="flex items-center justify-center gap-2">
                    <MapPin className="h-4 w-4" />{" "}
                    {upcomingMatch.location || "TBD"}
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-card/50 backdrop-blur-sm">
              ...
              <CardContent>
                <p className="text-muted-foreground">
                  There are no matches scheduled for you at the moment. Check
                  the main Draw for tournament updates.
                </p>
              </CardContent>
            </Card>
          )}
          ...
          <Card className="bg-card/50 backdrop-blur-sm">
            ...
            <CardContent className="grid gap-2">
              <Button asChild variant="outline">
                <Link href="/draw">View Full Draw</Link>
              </Button>
              <Button variant="outline" disabled>
                Edit Profile (Coming Soon)
              </Button>
              <Button variant="outline" disabled>
                Setup Notifications (Coming Soon)
              </Button>
            </CardContent>
          </Card>
        </div>
