...
async function main() {
  ...
  const oldMatches = JSON.parse(
    fs.readFileSync("fixtures.json", "utf8"),
  ) as OldMatch[];

  // --- Use a Map keyed by player ID for efficient O(1) lookups ---
  const playerMap = new Map<string, OldPlayer>(
    oldPlayers.map((p) => [p.id, p]),
  );
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      for (const oldPlayer of oldPlayers) {
        if (!oldPlayer.email || !oldPlayer.name) {
          console.error(
            `Skipping user with ID ${oldPlayer.id} due to missing name or email.`,
          );
          userSkips++;
          continue;
        }
        try {
          ...
          console.error(
            `Failed to migrate user ${oldPlayer.id} (${oldPlayer.name}):`,
            e,
          );
          userFails++;
        }
      ...
      for (const oldPlayer of oldPlayers) {
        try {
          ...
          console.error(
            `Failed to migrate player profile for ${oldPlayer.id}:`,
            e,
          );
          playerFails++;
        }
      ...
      for (const oldMatch of oldMatches) {
        ...
        const winner = oldMatch.winnerId
          ? playerMap.get(oldMatch.winnerId)
          : undefined;

        if (oldMatch.winnerId && !winner) {
          console.warn(
            `Match ${oldMatch.id} has winnerId ${oldMatch.winnerId}, but no matching player was found by ID. Winner will be set to undefined.`,
          );
        }

        if (!player1)
          console.log(
            `No player1 for match ${oldMatch.id}, searched ID: ${oldMatch.player1Id} (type: ${typeof oldMatch.player1Id})`,
          );
        if (!player2)
          console.log(
            `No player2 for match ${oldMatch.id}, searched ID: ${oldMatch.player2Id}`,
          );
        ...
        if (player1 && player2) {
          try {
            ...
            if (oldMatch.scheduledTime) {
              ...
              if (isNaN(parsedDate.getTime())) {
                throw new Error(
                  `Invalid date format: ${oldMatch.scheduledTime}`,
                );
              }
            ...
            if (oldMatch.winnerId && winner) {
              ...
              (oldMatch.scorePlayer1 ?? 0) > 0 ||
              (oldMatch.scorePlayer2 ?? 0) > 0
            ) {
              status = "IN_PROGRESS";
            }
            ...
            await tx.match.create({
              data: {
                ...
                round:
                  roundMap[oldMatch.round.toUpperCase() as OldRound] ??
                  "ROUND_1",
          ...
          console.warn(
            `Skipping match ${oldMatch.id} due to missing player(s) by ID lookup.`,
          );
          matchSkips++;
        }
      }
      ...
      if (userFails > 0 || playerFails > 0 || matchFails > 0) {
        throw new Error(
          `Migration had failures: Users ${userFails}, Players ${playerFails}, Matches ${matchFails}. Rolling back transaction.`,
        );
      }
    });
    ...
    console.error(
      "The migration transaction failed and has been rolled back.",
      error,
    );
  } finally {
    console.log("\n--- MIGRATION SUMMARY ---");
    console.log(
      `Users: ${userSuccess} succeeded, ${userFails} failed, ${userSkips} skipped out of ${oldPlayers.length} total.`,
    );
    console.log(
      `Players: ${playerSuccess} succeeded, ${playerFails} failed out of ${oldPlayers.length} total.`,
    );
    console.log(
      `Matches: ${matchSuccess} succeeded, ${matchFails} failed, ${matchSkips} skipped out of ${oldMatches.length} total.`,
    );
    console.log("-------------------------\n");
