...
export async function registerPlayer(prevState: ActionState, formData: FormData): Promise<ActionState> {
  ...
  try {
    ...
    if (approved.mobile !== mobile) {
      return { error: 'The mobile number does not match the one on file for this player.' };
    }
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === 'P2002') {
        return { error: 'This player or mobile number is already in the system. Please sign in.' };
      }
    }
