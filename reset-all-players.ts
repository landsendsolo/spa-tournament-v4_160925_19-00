...
async function resetAllPlayers() {
  ...
  for (const [name, { mobile, code }] of playerMap) {
    ...
    if (!existingPlayer) {
      ...
      console.log(`Player record already exists for ${name} with status: ${existingPlayer.status}`);
    }
  }

  console.log('Player registration reset completed for all players in playerMap.');
