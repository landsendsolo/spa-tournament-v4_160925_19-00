...
export function MatchListItem({ match }: { match: MatchWithPlayers }) {
  ...
  const getStatusBadge = () => {
    if (status === "IN_PROGRESS") {
      return (
        <div className="absolute top-2 right-2 text-xs font-bold text-red-500 bg-red-500/10 px-2 py-1 rounded-full animate-pulse">
          LIVE
        </div>
      );
    }
        ...
        <CardContent className="space-y-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              <Avatar className="h-8 w-8">
                <AvatarImage
                  src={player1?.user?.image ?? ""}
                  alt={player1?.user?.name ?? "Player 1"}
                />
                <AvatarFallback>
                  {player1?.user?.name?.charAt(0)}
                </AvatarFallback>
              </Avatar>
              <span className="font-medium">
                {player1?.user?.name ?? "TBD"}
              </span>
            </div>
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              <Avatar className="h-8 w-8">
                <AvatarImage
                  src={player2?.user?.image ?? ""}
                  alt={player2?.user?.name ?? "Player 2"}
                />
                <AvatarFallback>
                  {player2?.user?.name?.charAt(0)}
                </AvatarFallback>
              </Avatar>
              <span className="font-medium">
                {player2?.user?.name ?? "TBD"}
              </span>
            </div>
