...
import {
  ...
  resetMatch,
  undoFrame,
} from "@/app/actions/scoringActions";
...
export function LiveScorecard({
  ...
  const canScore =
    isAdmin ||
    (currentUserId &&
      (currentUserId === match.player1?.userId ||
        currentUserId === match.player2?.userId));
  const score1 = match.scorePlayer1 ?? 0;
  const score2 = match.scorePlayer2 ?? 0;
  const isMatchOver =
    score1 >= 7 || score2 >= 7 || match.status === "COMPLETED";
          ...
          <div className="flex flex-col sm:flex-row justify-around items-center text-center gap-4 sm:gap-0 mb-8">
            <div className="flex flex-col items-center gap-4">
              <Avatar className="h-24 w-24 border-4 border-muted">
                {match.player1?.user?.image && (
                  <AvatarImage
                    src={match.player1.user.image}
                    alt={match.player1.user.name ?? "Player 1"}
                  />
                )}
                <AvatarFallback>
            ...
            <div className="flex flex-col items-center gap-4">
              <Avatar className="h-24 w-24 border-4 border-muted">
                {match.player2?.user?.image && (
                  <AvatarImage
                    src={match.player2.user.image}
                    alt={match.player2.user.name ?? "Player 2"}
                  />
                )}
                <AvatarFallback>
          ...
          {isMatchOver ? (
            <div className="text-center pt-8 space-y-4">
              ...
              <p className="text-muted-foreground">
                The winner will advance to the next round automatically.
              </p>
            ...
            <div className="mt-8">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Button
                  ...
                  onClick={() => handleAwardFrame(match.player1Id)}
                  disabled={isPending}
                >
                  {isPending ? (
                ...
                <Button
                  size="lg"
                  className="py-8 text-lg"
                  onClick={() => handleAwardFrame(match.player2Id)}
                  disabled={isPending}
                >
                  {isPending ? (
                    <Loader2 className="mr-2 h-6 w-6 animate-spin" />
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-2 mt-4">
                <Button
                  variant="outline"
                  onClick={() => handleUndoFrame(match.player1Id)}
                  disabled={isPending || score1 === 0}
                  size="sm"
                >
                  <Undo2 className="mr-2 h-4 w-4" /> Undo{" "}
                  {match.player1?.user?.name}&apos;s last frame
                </Button>
                <Button
                  variant="outline"
                  onClick={() => handleUndoFrame(match.player2Id)}
                  disabled={isPending || score2 === 0}
                  size="sm"
                >
                  <Undo2 className="mr-2 h-4 w-4" /> Undo{" "}
                  {match.player2?.user?.name}&apos;s last frame
                </Button>
              </div>
            ...
            <p className="text-center text-muted-foreground mt-8">
              Players have not yet been assigned to this match.
            </p>
          )}
        </CardContent>
      </Card>
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This action will reset the match score to 0-0 and cannot be
                    undone.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
