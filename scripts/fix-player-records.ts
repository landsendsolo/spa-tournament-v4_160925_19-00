...
async function fixPlayerRecords() {
  console.log('Starting to fix Player records for all registered users with role PLAYER...');
  ...
  for (const [index, user] of users.entries()) {
    console.log(`[${index + 1}] User: ${user.name || 'Unnamed'} (ID: ${user.id})`);
    if (!user.player) {
      ...
      console.log(`Created Player record for ${user.name} with userId: ${user.id}`);
    } else {
      console.log(`Player record already exists for ${user.name} with status: ${user.player.status}`);
    }
  }
