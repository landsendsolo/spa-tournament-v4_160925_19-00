...
async function seedAdminUser() {
  ...
  console.log(
    "[OK] Admin user created successfully (email: admin@example.com, pass: password).",
  );
}
async function seedTournament() {
  ...
  if (existingTournament) {
    console.log(
      `[INFO] Tournament "${existingTournament.name}" already exists. Skipping.`,
    );
    return;
  }
...
async function seedPlayers() {
  ...
  const existingNames = new Set(
    existingUsers.map((u) => u.name).filter((name) => name !== null),
  );
  ...
  console.log(
    `[INFO] Seeding ${newPlayerNames.length} new players and placeholders...`,
  );
